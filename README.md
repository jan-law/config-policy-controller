...
| Field | Description |
| ---- | ---- |
| `compliant` | Overall compliance state: `Compliant` (operator meets all requirements), `NonCompliant` (operator does not meet requirements), `Pending` (evaluation in progress), or `Terminating` (policy is being deleted) |
...
spec:
  # (other fields not shown)
  upgradeApproval: Automatic # upgradeApproval controls the approval of install plans. Keep this field.
  subscription:
    namespace: default
    name: external-secrets-operator
    channel: alpha
    ...
    sourceNamespace: olm
    startingCSV: external-secrets-operator.v0.11.0
    installPlanApproval: Automatic # installPlanApproval is invalid. Remove this field.
status:
  compliant: NonCompliant
  conditions:
  # (other conditions not shown)
