	...
	describeTableArgs := []any{func(testPath string) {
		...
		DeferCleanup(func() {
			for _, resource := range files.inputResources {
				...
				utils.KubectlDelete("-f", resource)
			}
			...
			for _, nsFilePath := range files.inputNamespaces {
				...
				utils.KubectlDelete("-f", nsFilePath)
			}
		})

		Eventually(func(g Gomega) {
			applyNamespaces(g, files.inputNamespaces)
			applyResources(g, files.inputResources)
			verifyDryrunOutput(g, testPath, files.outputPath, files.desiredStatusPath, policyYAML, errOutputFile)
...
// applyNamespaces creates namespaces from input files, skipping managed namespaces
func applyNamespaces(g Gomega, inputNamespaces []string) {
	GinkgoHelper()
	const kubeconfig = "../../kubeconfig_managed_e2e"

	for _, nsFilePath := range inputNamespaces {
		isManagedNs, err := containsManagedNamespace(nsFilePath)
		if isManagedNs || err != nil {
			continue
		}
		...
		g.Expect(err).ToNot(HaveOccurred(), "Failed to create namespace: %s", string(output))
	}
}

// applyResources applies resource files to the cluster
func applyResources(g Gomega, inputResources []string) {
	GinkgoHelper()
	const kubeconfig = "../../kubeconfig_managed_e2e"

	for _, resource := range inputResources {
		By("Applying input file " + resource)
		cmd := exec.Command("kubectl", "apply", "-f", resource, "--kubeconfig="+kubeconfig)
		output, err := cmd.CombinedOutput()
		g.Expect(err).ToNot(HaveOccurred(), "Failed to apply resource: %s", string(output))
	}
}

// verifyDryrunOutput executes the dryrun command and compares actual vs expected output
func verifyDryrunOutput(g Gomega, testPath, outputPath, desiredStatusPath, policyYAML, errOutputFile string) {
	GinkgoHelper()
	...
	wantedErr := strings.HasSuffix(outputPath, errOutputFile)
